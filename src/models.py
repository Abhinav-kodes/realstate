...
MODEL_PARAMS = {
    "Random Forest": dict(
        ...
        params=dict(n_estimators=200, max_depth=12, min_samples_split=4,
...
@st.cache_data(show_spinner="Training all models (first run only) …")
def train_all_models(_df: pd.DataFrame):
    X = _df[FEATURE_COLS].copy()
    y = _df["price_per_area"].copy()
...
def get_shap(_model, _X_test: pd.DataFrame, feature_cols: list):
...
def predict_single(row_dict: dict, model, kmeans, scaler_geo,
                   cluster_price_map, median_date: float):
    import pandas as pd, numpy as np
    d = row_dict.copy()
    d["log_dist_mrt"]        = np.log1p(d["dist_mrt"])
    d["sqrt_dist_mrt"]       = np.sqrt(d["dist_mrt"])
    d["mrt_x_stores"]        = d["dist_mrt"] * d["convenience_stores"]
    d["dist_city_center_km"] = haversine(d["latitude"], d["longitude"], REF_LAT, REF_LON)
    d["accessibility_score"] = (d["convenience_stores"] * 10) / (np.log1p(d["dist_mrt"]) + 1)
    ...
    d["cluster_avg_price"]   = float(cluster_price_map.get(d["geo_cluster"], cluster_price_map.mean()))
